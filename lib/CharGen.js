...
app.controller('CharGenController', ['$scope', function ($scope) {
    ...
    $scope.learn = function (skill) {
        if(typeof skill === 'string') {
            if(!Savage.Skill[skill.name]) {
                skill = Savage.Skill[skillName]();
            } else {
                skill = new Savage.Skill(skillName, 'd4');
            }
        }
    ...
    function createEdgeFilter (character) {
        return function () {
            return Savage.Edges.filter(function (edge) {
                return !character.edges.contains(edge) && edge.isAvailableTo(character);
            });
        };
    }

    $scope.availableEdges = createEdgeFilter($scope.character);
